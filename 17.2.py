...
pq = PriorityQueue()
pq.put((0, start, None, 0))  # (distance, node, direction, steps_in_direction, prev_nodes)
    
while not pq.empty():
    dist, node, prev_direction, steps = pq.get()
    if node == end and steps >= 4:
        print(dist)
        break
    ...
    for d_index, d in enumerate(directions):
        ...
        if i_n < 0 or i_n >= len(grid) or j_n < 0 or j_n >= len(grid[0]):
            ...
            continue
        elif d_index == prev_direction and steps == 10:
            continue
        elif prev_direction != None and d_index != prev_direction and steps < 4:
            continue
